...
> Repository slug is still `git-dashboard` in some local clones; the canonical slug is now `landingit`. iOS bundle identifier and Electron `appId` are `com.github.adamsilverstein.landingit`, matching the App Store Connect registration (Apple ID 6765520480).
